...
prmtrcNGC :: [Polyline] -> Text
prmtrcNGC polylines = toLazyText $ gcodeHeader <> mconcat (map interpretPolyline orderedPolylines) <> gcodeFooter
    where 
      orderedPolylines =
            snd . unzip 
            . List.sortBy (\(a,_) (b, _) -> compare a b)
      ...
      gcodeHeader :: Builder
      gcodeHeader = mconcat [
                     "(generated by ImplicitCAD, prmtrcNGC)\n"
                    ,"#<Zsafe>=2      (Change if required)"
                    ,"#<Zngrv>=-0.2  (Change if required)"
                    ,"#<Zcut>=-3.0    (Change if required)"
                    ,"#<FeedRate>=400 ()"
                    ,"G21 (units=mm)\n"
                    ,"F#<FeedRate>\n"
                    ," "
                    ,"G0 Z#<Zsafe>\n"
                    ,"M3 S1 (enable laser or mill)\n\n"]
      gcodeFooter :: Builder
      gcodeFooter = mconcat [
                     "M5 (disable laser)\n"
                    ,"G00 X0.0 Y0.0 (move to 0)\n"
